...
namespace Timeline.Controls
{
    ...
    public partial class TimelineControl2 : ContentView
    {
        ...
        {
            ((TimelineControl2)bindable).initialOrientationCheck = true;
            ((TimelineControl2)bindable).InvalidateLayout();
        }
        ...
        {
            ((TimelineControl2)bindable).date = (TLDate)newValue;
            ((TimelineControl2)bindable).InvalidateLayout();
        }
        ...
        public TLDatePrecision ZoomUnit
        {
            get { return (TLDatePrecision)GetValue(ZoomUnitProperty); }
            set { SetValue(ZoomUnitProperty, value); }
        }

        public TLDate Date
        {
            get { return (TLDate)GetValue(DateProperty); }
        ...
        public TimelineControl2()
        {
            ...
            date = new TLDate(2018);
            unitDate = new TLDate(2018);
            subUnitDate = new TLDate(2018);
            DateStr = date.DateStrFull; //date.DateStr(ZoomUnit);

            pixeltime = Zoom; // (long)(Zoom * TimeSpan.TicksPerSecond);
        ...
        void GestureRecognizer_OnGestureRecognized(object sender, TouchGestureEventArgs args)
        {
            switch (args.Type)
            {
                ...
                case TouchGestureType.Pan:
                    ...
                    switch (ZoomUnit)
                    {
                        case TLDatePrecision.Minute: date.AddMinutes((int)(args.Data.X * Zoom)); break;
                        case TLDatePrecision.Hour: date.AddHours((int)(args.Data.X * Zoom)); break;
                        case TLDatePrecision.Day: date.AddDays((int)(args.Data.X * Zoom)); break;
                        case TLDatePrecision.Month: date.AddMonths((int)(args.Data.X * Zoom)); break;
                        case TLDatePrecision.Year: date.AddYears((int)(args.Data.X * Zoom)); break;
                        case TLDatePrecision.KYear: date.AddYears((int)(args.Data.X * Zoom)*1000); break;
                        case TLDatePrecision.KKYear: date.AddYears((int)(args.Data.X * Zoom)*10000); break;
                        case TLDatePrecision.KKKYear: date.AddYears((int)(args.Data.X * Zoom)*100000); break;
                        case TLDatePrecision.MYear: date.AddYears((int)(args.Data.X * Zoom)*1000000); break;
                        case TLDatePrecision.KMYear: date.AddYears((int)(args.Data.X * Zoom)*10000000); break;
                        case TLDatePrecision.KKMYear: date.AddYears((int)(args.Data.X * Zoom)*100000000); break;
                    }
                    
                    //date.BaseDate = date.BaseDate.AddTicks(-2 * (long)args.Data.X * pixeltime);
                    //DateStr = date.DateStr(ZoomUnit - 1);
                    DateStr = date.DateStrFull;
                    canvasView.InvalidateSurface();
                    break;

                case TouchGestureType.Pinch:
                    //Console.WriteLine("PINCH - " + args.Data.ToString());
                    Zoom -= Zoom * 0.005f * args.Data.X;
                    if (Zoom < 4) Zoom = 4;
